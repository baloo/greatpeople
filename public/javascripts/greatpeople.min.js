...
l){m=f("<option />").val(l).append(q[l]);f("select",this).append(m)}f("select",this).children().each(function(){if(f(this).val()==q.selected||f(this).text()==f.trim(d.revert))f(this).attr("selected","selected")})}}},addInputType:function(m,h){f.editable.types[m]=h}};f.fn.editable.defaults={name:"value",id:"id",type:"text",width:"auto",height:"auto",event:"click.editable",onblur:"cancel",loadtype:"GET",loadtext:"Loading...",placeholder:"Click to edit",loaddata:{},submitdata:{},ajaxoptions:{}}})(jQuery);(function(f){var m=Backbone.Model.extend({initialize:function(){this.set({stars:this.get("rating")?this.get("rating")*100/4:0})}}),h=new (Backbone.Collection.extend({model:m,box:"new",url:"/api/applicants",query:null,page:0,pageCount:1,parse:function(l){this.page=l.pageNumber;this.pageCount=l.pageCount;return l.applications}})),d=Backbone.View.extend({tagName:"dl",className:"candidate-list",render:function(){f("#applicantTmpl").tmpl(this.model.toJSON()).appendTo(this.el);return this}}),q=Backbone.View.extend({el:f("#applist")[0],
initialize:function(){_.bindAll(this,"render");h.bind("reset",this.render)},refreshLinks:function(l){var n=l?"/"+l.split(",")[0]:"";f(["new","inprogress","archived"]).each(function(){f(".navlink."+this+" a").attr("href","#"+this+n)})},spinner:function(){f("#spinner").show()},render:function(){f("#spinner").hide();var l=f(this.el).html("");if(h.length===0)l.append(f("#noApplicantTmpl").tmpl());else{var n=h.box;h.map(function(u){return new d({model:u})}).forEach(function(u){l.append(u.render().el)});
f(".tags > ul > li > a").each(function(){f(this).attr("href","#"+n+"/tag:"+f(this).attr("data"))});var t=f("#pagination");t.html("");h.pageCount>1&&_(h.pageCount).times(function(u){t.append(h.page==u?u+1:f("#pageLink").tmpl({page:u+1,link:"#"+h.box+"/"+(h.query?h.query:"")+","+(u+1)}));t.append("&nbsp;")});f("#pagination").toggle(h.pageCount>1)}},login:function(){window.location="/login?callback="+encodeURIComponent(window.location)}});m=Backbone.Router.extend({routes:{"":"index","new":"newapp","new/:query":"newapp",
inprogress:"inprogress","inprogress/:query":"inprogress",archived:"archived","archived/:query":"archived"},initialize:function(){this.view=new q({})},index:function(){window.location.hash="#new"},newapp:function(l){this._showBox("new",l)},inprogress:function(l){this._showBox("inprogress",l)},archived:function(l){this._showBox("archived",l)},_showBox:function(l,n){this.box=l;f("#search").val(n?n.split(",")[0]:"");h.box=this.box;h.url="/api/applicants/"+this.box;if(n){h.query=n;h.page=1;if(n.indexOf(",")>
-1){h.query=n.split(",")[0];h.page=n.split(",")[1]}h.url+="?q="+encodeURIComponent(h.query)+"&p="+h.page}else{h.query="";h.page=1}h.fetch();f(".navlink a").removeClass("active");f("."+l+" a").addClass("active")}});window.Workspace=m})(jQuery);(function(f){var m=function(h,d){this.resumeId=h;this.options=f.extend({allTags:[],userTags:[]},d);this.options.post&&this.initInplaceEdit();this.initTabBar();this.setTagBox()};m.prototype={initInplaceEdit:function(){var h=this;f(".inplace").each(function(){f(this).editable(h.options.post,{tooltip:"Click to edit",attribute:this.id,style:"display: inline",submitdata:{resumeId:h.resumeId},width:this.id=="name"?"auto":100,onsubmit:function(){h.options.notification&&h.options.notification.display("Saving...")},
